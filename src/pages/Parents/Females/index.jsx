  ...
  useEffect(() => {
    async function fetchStudData() {
      try {
        ...
        setStud(studData);
        setOtherStuds(allParents.filter(p => p.id !== parseInt(id) && p.class_list.includes("category-male")));
      } catch (error) {
          ...
          <div className={styles.right}>
            ...
            <div className={styles.details}>
              <div>
                ...
                <span className={styles.detright}>: {getGenderFromClassList(stud.class_list)}</span>
              </div>
              <div>
                <span className={styles.detleft}>Age</span>
                <span className={styles.detright}>: {calculateAgeDisplay(stud.acf.age)}</span>
              </div>
              <div>
                <span className={styles.detleft}>Size</span>
                <span className={styles.detright}>: {getSizeFromClassList(stud.class_list)}</span>
              </div>
              <div>
